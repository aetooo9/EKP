...
if(!empty($_GET['SubMenuID'])){
$SubMenuID=$_GET['SubMenuID'];
$sql = "select * from SubMenus where SubMenuID=?";
$r = $conn->prepare($sql);
$r->bind_param("i",$SubMenuID);
$r->execute();
$rr = $r->get_result();
$SubMenuRow = $rr->fetch_assoc();
}
  ...
  <nav class="navbar navbar-vertical fixed-right navbar-expand-md navbar-light bg-white" id="sidenav-main">
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="sidenav-collapse-main">
        ...
        <ul class="navbar-nav">
          <li class="nav-item   ">
            <a class="nav-link   " href="MenuAdd.php">
              <i class="ni ni-bullet-list-67 text-primary"></i> إضافة قائمة رئيسية
            </a>
          </li>
          <li class="nav-item  active ">
            <a class="nav-link  active " href="SubMenuAdd.php">
        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header bg-transparent">
              <h3 class="mb-0">إضافة قائمة فرعية</h3>
            </div>
            <div class="card-body">
              <div class="row icon-examples">
                <div class="col-lg-12 col-md-12">
                    <form method="post">
                <input type="hidden" name="SubMenuID" value="<?php echo $SubMenuRow['SubMenuID']; ?>">
                <input type="text"  name="SubMenuName" class="form-control " value="<?php echo $SubMenuRow['SubMenuName']; ?>" placeholder="إسم القائمة الفرعية">
<br>
                ...
                ?>
                <option value="<?php echo $row['MenuID']; ?>" <?php if($SubMenuRow['MenuID']==$row['MenuID']) echo "selected";?>><?php echo $row['MenuName']; ?></option>
                <?php
                }
                ?>
                </select>
<br>
                <textarea class="form-control" name="SubMenuContent" id="editor1" ><?php echo $SubMenuRow['SubMenuContent']; ?></textarea>
                <br>
                <?php
                if(!empty($_GET['SubMenuID'])){
                    ?>
                ...
                if(isset($_POST['save'])){
                    ...
                    $SubMenuContent=$_POST['SubMenuContent'];
                    
                    $sql = "insert into SubMenus(MenuID,SubMenuName,SubMenuContent) values(?,?,?)";
                    $r = $conn->prepare($sql);
                    $r->bind_param("iss",$MenuID,$SubMenuName,$SubMenuContent);
                    $e = $r->execute();
                    if($e){
                        echo "<script>alert('تم الاضافة نجاح');</script>";
                        echo "<script>location.href='SubMenuAdd.php';</script>";
                   }else{
                    ...
                    if(isset($_POST['upd'])){
                        ...
                        $sql = "update SubMenus set MenuID=?,SubMenuName=?,SubMenuContent=? where SubMenuID=?";
                        $r = $conn->prepare($sql);
                        $r->bind_param("issi",$MenuID,$SubMenuName,$SubMenuContent,$SubMenuID);
                        $e = $r->execute();
                        if($e){
                    ...
                    if(isset($_POST['del'])){
                        ...
                        $sql = "delete from SubMenus where SubMenuID=?";
                        $r = $conn->prepare($sql);
                        $r->bind_param("i",$SubMenuID);
                        $e = $r->execute();
                    if($e){
        ...
        <?php
          $sql = "select s.SubMenuID,s.SubMenuName,m.MenuName from SubMenus s
                  inner join Menus m on m.MenuID=s.MenuID ";
                  $r = $conn->prepare($sql);
                  $r->execute();
                  $rr = $r->get_result();
          $i=0;
        ...
        <td><a href="SubMenuAdd.php?SubMenuID=<?php echo $row['SubMenuID'];?>">تعديل</a></td>
        </tr>
        <?php
          }
          ?>
